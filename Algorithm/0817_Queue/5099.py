...
for tc in range(1, T+1):
    ...
    pizzas = []
    for i in range(M):
        pizzas.append((Ci[i], i + 1))
    queue = [0]*N
    front = 0
    rear = 0
    rear += 1
    pizza = 0
    queue[rear] = pizzas[pizza]
    while front != rear:
        if (rear + 1) % N != (front + 1) % N and pizza < M - 1:
            rear += 1
            pizza += 1
            queue[rear % N] = pizzas[pizza]
        else:
            front += 1
            now = queue[front % N][0] // 2
            piz_num = queue[front % N][1]
            if now != 0:
                queue[front % N] = (now, piz_num)
                rear += 1
            elif piz_num == 0:
                rear += 1
            else:
